// Mock backend functionality for ResumeGenie
...
const generateReferralCode = (name: string) => {
  return (name.substring(0, 3) + Math.random().toString(36).substr(2, 5)).toUpperCase();
};
...
export const auth = {
  signup: (name: string, email: string, password: string, referralCode?: string) => {
    ...
    pointsService.addTransaction(newUser.id, 1, 'welcome', 'Welcome bonus - 1 free point!');
    ...
    notificationService.create(newUser.id, {
      type: 'success',
      title: 'Welcome to ResumeGenie! 🎉',
      message: 'You have 1 free point to try our CV fix service. Upload your CV to get started!'
    });
...
const processReferral = (code: string, referredEmail: string) => {
  ...
  if (referrer) {
    ...
    pointsService.addPoints(referrer.id, 2, 'referral', `Referral bonus - ${referredEmail} joined!`);
  }
};

// Points service
export const pointsService = {
  addPoints: (userId: string, amount: number, type: string, description: string) => {
  ...
  deductPoints: (userId: string, amount: number, type: string, description: string) => {
    const user = auth.getCurrentUser();
    if (!user || user.id !== userId) return;
  ...
  addTransaction: (userId: string, amount: number, type: string, description: string) => {
  ...
  getTransactions: (userId: string) => {
    ...
    return transactions.filter((t: PointsTransaction) => t.userId === userId)
      .sort((a: PointsTransaction, b: PointsTransaction) => 
  ...
  purchasePoints: (userId: string, packagePrice: number, points: number) => {
    ...
    pointsService.addPoints(userId, points, 'purchase', `Purchased ${points} points for ₦${packagePrice.toLocaleString()}`);
...
export const cvService = {
  ...
  getAll: (userId: string) => {
    return storage.getUserItems('cvs').sort((a: CVDocument, b: CVDocument) => 
      new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
    );
  },
...
export const coverLetterService = {
  create: async (userId: string, jobTitle: string, company: string, jobDescription: string) => {
  ...
  getAll: (userId: string) => {
    return storage.getUserItems('coverLetters').sort((a: CoverLetter, b: CoverLetter) => 
      new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
    );
  },
...
export const interviewPrepService = {
  ...
  getAll: (userId: string) => {
    return storage.getUserItems('interviewPreps').sort((a: InterviewPrep, b: InterviewPrep) => 
      new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
    );
  },
  
  getById: (id: string) => {
    return storage.get('interviewPreps')?.find((p: InterviewPrep) => p.id === id);
  }
};
...
export const jobApplicationService = {
  ...
  updateStatus: (id: string, status: JobApplication['status'], notes?: string) => {
    return storage.updateItem('jobApplications', id, { status, notes });
  },
  
  getAll: (userId: string) => {
    return storage.getUserItems('jobApplications').sort((a: JobApplication, b: JobApplication) => 
      new Date(b.appliedAt).getTime() - new Date(a.appliedAt).getTime()
    );
  },
  
  getById: (id: string) => {
    return storage.get('jobApplications')?.find((a: JobApplication) => a.id === id);
  }
};
...
export const notificationService = {
  ...
  getAll: (userId: string) => {
    return storage.getUserItems('notifications').sort((a: Notification, b: Notification) => 
      new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
    );
  },
...
export const referralService = {
  getReferrals: (userId: string) => {
    ...
    return referrals.filter((r: Referral) => r.referrerId === userId)
  ...
  getTotalEarned: (userId: string) => {
    ...
    return referrals.reduce((sum: number, r: Referral) => sum + r.pointsEarned, 0);
  }
};
...
function generateCoverLetter(jobTitle: string, company: string, jobDescription: string): string {
  const user = auth.getCurrentUser();
...
function generateInterviewQuestions(jobTitle: string, jobDescription: string): InterviewQuestion[] {
  return [
    {
    ...
    },
    {
      ...
      suggestedAnswer: 'My greatest strengths include problem-solving, adaptability, and strong communication skills. I consistently deliver results under pressure and work well in team environments.',
      tips: [
    ...
    },
    {
      ...
      suggestedAnswer: 'In my previous role, I faced a tight deadline on a critical project. I organized the team, prioritized tasks, and we successfully delivered on time with high quality results.',
      tips: [
    ...
    const sampleJobs = [
      {
        ...
        requirements: ['5+ years experience', 'React & Node.js', 'Team leadership', 'Excellent communication'],
        posted: '2 days ago'
      },
      {
        id: generateId(),
        ...
        description: 'Join our team as a Product Manager to drive innovation in financial technology solutions.',
        requirements: ['3+ years PM experience', 'Agile methodology', 'Stakeholder management', 'Data-driven'],
        posted: '1 week ago'
      },
      {
        id: generateId(),
        ...
        description: 'Creative marketing specialist needed to develop and execute marketing campaigns.',
        requirements: ['2+ years experience', 'Digital marketing', 'Content creation', 'Analytics'],
        posted: '3 days ago'
      }
    ];
